...
Upload/re-upload release binaries for an existing version tag:

```bash
make release-assets VERSION=0.2.0
```
...
- For first publish of scoped packages (for example `@susu-eng/tender`), the npm user behind the token must have publish rights in that scope/org.
- `gh` must be authenticated with permission to create/upload GitHub releases.
